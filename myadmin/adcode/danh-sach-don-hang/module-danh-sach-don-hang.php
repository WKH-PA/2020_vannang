...
if (isset($_GET['them-moi']) || (isset($_GET['edit']) && is_numeric($_GET['edit']))) {
    ...
    $starDate = isset($_GET['star_date']) ? $_GET['star_date'] : "";
    $endDate = isset($_GET['end_date']) ? $_GET['end_date'] : "";
    $starSearch = date('d/m/Y', time());
    $endSearch = date('d/m/Y', strtotime("+1 day"));
    ...
    if (!empty($starDate)) {
        $tempStar = explode('/', $starDate);
        $starSearch = date('d/m/Y', strtotime($tempStar[2] . "/" . $tempStar[1] . "/" . $tempStar[0]));

        $mo .= ' AND `ngaydat`>=' . strtotime($tempStar[2] . "/" . $tempStar[1] . "/" . $tempStar[0]) . ' ';
        $uri .= '&star_date=' . $starDate;
    }

    if (!empty($endDate)) {
        $tempStar = explode('/', $endDate);
        $endSearch = date('d/m/Y', strtotime($tempStar[2] . "/" . $tempStar[1] . "/" . $tempStar[0]));
        $mo .= ' AND `ngaydat`<=' . strtotime($tempStar[2] . "/" . $tempStar[1] . "/" . $tempStar[0] . " 23:59:59") . ' ';
                                    ...
                                    <div class="form-group">
                                        <div>
                                            <div class="input-group date"
                                                 style="display: inline-block; width: 200px; float: left;">
                                                <div class="input-group-addon"
                                                     style="width: 38px; float: left; height: 32px;">
                                                    <i class="fa fa-calendar"></i>
                                                </div>
                                                <input name="" type="text" class="form-control date_bg datepicker"
                                                       onchange='SEARCH_jsstep()'
                                                       id="datepicker" value="<?= $starSearch ?>"
                                                       style="width: calc(100% - 48px); float: left; border-radius: 0; margin-right: 10px; border: 1px solid #d2d6de;">
                                            </div>
                                            <div class="input-group date"
                                                 style="display: inline-block; width: 200px; float: left;">
                                                <div class="input-group-addon"
                                                ...
                                                <input name="" type="text" class="form-control date_end datepicker"
                                                       onchange='SEARCH_jsstep()'
                                                       id="datepicker2" value="<?= $endSearch ?>"
                                                       style="width: calc(100% - 48px); float: left; border-radius: 0; margin-right: 10px; border: 1px solid #d2d6de;">
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <tr style="<?= $cus_del == 1 ? "text-decoration: line-through" : "" ?>">
                                        ...
                                        <td>
                                            <select name="status_<?= $cl ?>" class="form-control"
                                                    onchange="UPDATE_colum(this, '<?= $ida ?>', 'trangthai','<?= $table ?> ')">
                                                <option value="1" <?= LAY_selected(1, $trang_thai) ?>>Đơn hàng mới
                                                </option>
                                                <option value="2" <?= LAY_selected(2, $trang_thai) ?>>Đang xử lý
                                                </option>
                                                <option value="3" <?= LAY_selected(3, $trang_thai) ?>>Đã giao hàng
                                                </option>
                                                <option value="4" <?= LAY_selected(4, $trang_thai) ?>>Hủy đơn hàng
                                                </option>
                                            </select>
                                        </td>
                                        <td class="text-center">
                                            <label><input showhi type='checkbox' class='minimal minimal_click'
                                                          colum="thanh_toan" idcol="<?= $ida ?>" table="<?= $table ?>"
                                                          value='1' <?= LAY_checked($thanh_toan, 1) ?>></label>
                                        </td>
                                        <!-- <td class="text-center">
        ...
        function SEARCH_jsstep() {
            setTimeout(function () {
                ...
                if ($("#datepicker").length > 0) {
                    var js_datepicker = $("#datepicker").val().trim();
                    if (js_datepicker !== "") url += "&star_date=" + js_datepicker;
                }
                if ($("#datepicker2").length > 0) {
                    var js_datepicker2 = $("#datepicker2").val().trim();
                    if (js_datepicker2 !== "") url += "&end_date=" + js_datepicker2;
                }

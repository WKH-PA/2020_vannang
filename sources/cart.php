                    ...
                    <tr class="tr_cart_iscart_1202">
                                ...
                                <form action="" method="post" >
                                    ...
                                    <button class="del cur" type="submit" name="xoa_sp"
                                            ...
                                            title="Xóa"><i class="fa fa-trash"
                                                           aria-hidden="true"></i> <?= $glo_lang['cart_xoa'] ?></button>
                                </form>
                            </div>
                        </td>
                        ...
                        <td class="qty c-th-sluong" style="width: 145px">
                            <div class="so-luong-mua">
                                <span class="number-down" onclick="add_num_sp('#product-quantity-<?=$id_sp?>',-1); updateQty_notthis('<?=$full_url?>/update-qty/', '<?=$id_sp?>');">–</span>
                                <input onchange="updateQty('<?=$full_url?>/update-qty/','<?=$id_sp?>', this)" type="text" class="product-quantity" value="<?=$value?>" id="product-quantity-<?=$id_sp?>">
                                <span class="number-up" onclick="add_num_sp('#product-quantity-<?=$id_sp?>',+1); updateQty_notthis('<?=$full_url?>/update-qty/', '<?=$id_sp?>');">+</span>
                            </div>
                        </td>
                        <td class="c-th-ttien" title="Thành tiền"><b class="td_thanhtien_<?=$id_sp?>"><?=NUMBER_fomat_d($thanhtien)?></b>
                        </td>
                    </tr>
                <?php }
            } ?>
        ...
        <div class="checkout-type-button-cart" style="text-align: center;">
            <div class="method-button-cart">
                <div class="sc_item_button sc_button_wrap sc_align_left">
                    <a href="<?=$full_url.'/dat-hang'?>" id="sc_button" class="sc_button"><?=$glo_lang['thanh_toan']?>
                    </a><!-- /.sc_button -->
                </div>
            </div>
        </div>
